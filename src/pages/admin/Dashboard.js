...
function Dashboard() {
  ...
  // Tạo danh sách 5 năm gần nhất để chọn
  ...
  // --- Logic tính toán chỉ số tổng quan  ---
  const { stats, topProducts } = useMemo(() => {
    ...
    return {
      stats: {
        totalRevenue: revenue, paidCount, unpaidCount, totalMoney,
        denied, totalNumberProductSale
      }, topProducts: sortedProducts
    };
  }, [orders]);

  // --- Logic xử lý dữ liệu biểu đồ đường ---
  const chartDataConfig = useMemo(() => {
    const now = new Date();
    let startDate, endDate, labels, getLabelKey;

    if (timeFilter === 'custom') {
      // Logic tùy chỉnh ngày
      ...
      // Logic chọn năm cụ thể
      const yearDate = new Date(selectedYear, 0, 1); // Ngày 1 tháng 1 của năm đã chọn
      startDate = startOfYear(yearDate);
      endDate = endOfYear(yearDate);
      
      // Label hiển thị 12 tháng
  ...
  }, [orders, timeFilter, customStartDate, customEndDate, selectedYear]); // Nhớ thêm selectedYear vào dependency
    ...
    <div className='md:px-8 py-6 xl:py-8 m-1 sm:m-3 h-[97vh] overflow-y-scroll w-full lg:w-11/12 bg-slate-50 shadow rounded-xl'>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          ...
          {/* [MỚI] Hiển thị input chọn năm khi mode = year */}
          {timeFilter === 'year' && (
             <div className='flex items-center gap-2 border rounded-lg px-2 py-1.5 bg-white text-xs shadow-sm'>
                ...
                <select 
                   ...
                   className='bg-transparent font-bold outline-none cursor-pointer text-blue-600'
                >
                   {yearsList.map(year => (
          ...
          {/* Hiển thị input chọn ngày khi mode = custom */}
          {timeFilter === 'custom' && (
          ...
          <div className='flex items-center gap-2 border rounded-lg px-3 py-1.5 bg-white text-xs shadow-sm'>
            ...
            <select
              ...
              className='bg-transparent font-bold outline-none cursor-pointer text-gray-700'
            >
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        ...
        <div className='lg:col-span-2 bg-white p-6 rounded-xl border shadow-sm flex flex-col'>
          <h3 className='font-bold mb-6 text-gray-700 uppercase text-sm tracking-wider'>
             Biểu đồ doanh số {timeFilter === 'year' ? `năm ${selectedYear}` : ''}
          </h3>
